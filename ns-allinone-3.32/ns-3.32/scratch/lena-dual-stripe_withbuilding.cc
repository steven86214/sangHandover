...
void LteUePhyReportUeMeasurementsCb(std::string context, uint16_t rnti, uint16_t cellId, double rsrp, double rsrq, bool isServingCell, uint8_t componentCarrierId)
{
    ...
    std::cout << "nodeId : " << nodeId << ", cellId" << cellId << ", rsrp " << rsrp << ", rsrq " << rsrq << std::endl;
...
int main(int argc, char *argv[])
{
    ...
    else
    {
        ...
        xVal->SetAttribute("Min", DoubleValue(100));
        xVal->SetAttribute("Max", DoubleValue(101));
        positionAlloc->SetAttribute("X", PointerValue(xVal));
        Ptr<UniformRandomVariable> yVal = CreateObject<UniformRandomVariable>();
        yVal->SetAttribute("Min", DoubleValue(149));
        yVal->SetAttribute("Max", DoubleValue(150));
        ...
        // nodePosAlloc->Add(Vector(50.0, 50.0, 0.0));
            ...
            for (uint32_t b = 0; b < numBearersPerUe; ++b)
            {
                ...
                if (useUdp)
                {
                    if (epcDl)
                    {
                        ...
                        // serverApps.Get(0)->TraceConnectWithoutContext("Rx", MakeBoundCallback(&TraceRx,0));
                    }
                    if (epcUl)
                    {
    ...
    // animation
    AnimationInterface anim("dual-stripe-withbuilding.xml");
    anim.UpdateNodeColor(remoteHost, 255, 0, 0);
    anim.UpdateNodeSize(remoteHost->GetId(), 5.0, 5.0);
    for (uint32_t i = 0; i < homeEnbs.GetN(); ++i)
    {
        anim.UpdateNodeColor(homeEnbs.Get(i), 0, 255, 0);
        anim.UpdateNodeSize(homeEnbs.Get(i)->GetId(), 5.0, 5.0);
    }
    for (uint32_t i = 0; i < macroEnbs.GetN(); ++i)
    {
        anim.UpdateNodeColor(macroEnbs.Get(i), 255, 255, 0);
        anim.UpdateNodeSize(macroEnbs.Get(i)->GetId(), 5.0, 5.0);
    }
    for (uint32_t i = 0; i < homeUes.GetN(); ++i)
    {
        anim.UpdateNodeColor(homeUes.Get(i), 0, 0, 255);
        anim.UpdateNodeSize(homeUes.Get(i)->GetId(), 5.0, 5.0);
    }
    for (uint32_t i = 0; i < macroUes.GetN(); ++i)
    {
        anim.UpdateNodeColor(macroUes.Get(i), 0, 255, 255);
        anim.UpdateNodeSize(macroUes.Get(i)->GetId(), 5.0, 5.0);
    }
    ...
    std::cout << "homeEnb" << homeEnbs.GetN() << " macroEnbs" << macroEnbs.GetN() << " homeUes" << homeUes.GetN() << " macroUes" << macroUes.GetN();
    lteHelper = 0;
    Simulator::Destroy();
    return 0;
}
